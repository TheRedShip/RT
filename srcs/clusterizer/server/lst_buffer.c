...
/*   Updated: 2024/05/17 15:23:32 by tomoron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*buffer_to_str(t_buffer *buffer, int srv, t_scene *scene)
{
	char	*res;
	long	len;
	...
	if ((srv && len != ((long)((WIDTH * HEIGHT + 2) * sizeof(t_vec3f)) + 1))
		|| (!srv && len < (long)(sizeof(t_vec3f) * 2) + 1))
	{
		free_buffer(buffer);
		return (0);
	}
