...
int	rt_parse_camera(char *line, t_scene *scene)
{
	...
	scene->camera->direction = rt_atof3(split[2], -1.0f, 1.0f);
	scene->camera->direction.z = scene->camera->direction.x;
	scene->camera->direction.x = scene->camera->direction.y * (M_PI);
	scene->camera->direction.y = scene->camera->direction.z * (M_PI);
