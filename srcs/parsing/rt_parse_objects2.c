...
void scale_quad(t_vec3f *up, t_vec3f *right, t_vec3f *origin, float width, float height) {
    // Scale the vectors up and right
    up->x *= height;
    up->y *= height;
    up->z *= height;
    ...
    // Adjust the origin to maintain the position
...
int		rt_parse_portal(char *line, t_scene **scene)
{
	...
	portal = rt_add_objects(scene, "qd");
	portal->quad->normal = rt_atof3(split[2], -1.0, 1.0);
	portal->origin = rt_atof3(split[1], -1000.0, 1000.0);
	portal->origin = vec3f_add_v(portal->origin, vec3f_mul_f(portal->quad->normal,0.01));
	calculate_up_right(portal->quad->normal, &portal->quad->up_corner, &portal->quad->right_corner);
	scale_quad(&portal->quad->up_corner, &portal->quad->right_corner,&portal->origin, 2.5, 5.5);
	portal->quad->d = vec3f_dot(portal->quad->normal, portal->origin);
	portal->quad->w = vec3f_div_f(portal->quad->normal, vec3f_dot(portal->quad->normal,portal->quad->normal));
	portal->material.color = (t_vec3f){0.0, 0.0, 0.0};
	if (rt_parse_material("1.0,0.0", &(portal->material)) == -1)
		return (rt_return(split));
	ft_free_tab((void **)(split));
	return (1);
}