...
/*   Updated: 2024/05/03 16:05:21 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_boundary	get_quad_boundary(t_objects *object)
{
	...
	boundary.origin = vec3f_sub_v(object->origin, object->quad->normal);
	boundary.size = vec3f_add_v(vec3f_add_v(object->quad->right_corner, object->quad->up_corner), \
								vec3f_mul_f(object->quad->normal, 2.0f));
	return (boundary);
}
